...
async function pushEntityToViewer(entity_byte, quid) {
  ...
  if (entityInGraphCheck(quid)) {
    ...
    node_dataArray.forEach((node_data, i) => {
      ...
      let parentEntity = document.createElement("tcg-card");
      
      
      parentEntity.setAttribute('image_url', node_data.image_url.value);
      parentEntity.setAttribute('instanceof', node_data.instanceofLabel.value);
      parentEntity.setAttribute('label', node_data.quidLabel.value);
    ...
    linkedData.forEach((ent, i) => {
      let entityEl = document.createElement("tcg-card");
      entityEl.setAttribute('image_url', ent.level2_image_url.value);
      entityEl.setAttribute('instanceof', ent.level2InstanceOfLabel.value);
      entityEl.setAttribute('label', ent.level2NodeLabel.value);
      entityEl.setAttribute('connectionType', ent.propLabel.value.split("/").pop());
